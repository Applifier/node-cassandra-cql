...
Connection.prototype.open = function (callback) {
  ...
  this.netClient.once('timeout', function connectTimeout() {
    var err = new types.DriverError('Connection timeout (' + self.options.connectTimeout + ') to host ' + self.options.host + ':' + self.options.port);
...
Connection.prototype.execute = function () {
  ...
  this.sendStream(
    ...
    function () {
      ...
      self.latencyHistory.push(end - start);
      if (self.latencyHistory.length > 100) {
        self.latencyHistory.shift();
      }
      if (args.callback) {
...
/**
 * Calculates average request execution time based on last n requests in this.latencyHistory buffer
 */
Connection.prototype.getAverageLatency = function () {
  var sum = 0;
  var l = this.latencyHistory.length;
  for (var i = 0; i < l; i++) {
...
/**
 * Resets latency history
 */
Connection.prototype.resetLatencyHistory = function () {
  this.latencyHistory = [];
};
